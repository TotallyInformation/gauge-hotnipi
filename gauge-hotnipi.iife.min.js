(()=>{var x=Object.defineProperty;var k=(c,l,a)=>l in c?x(c,l,{enumerable:!0,configurable:!0,writable:!0,value:a}):c[l]=a;var h=(c,l,a)=>(k(c,typeof l!="symbol"?l+"":l,a),a);var f="gauge-hotnipi";function w(c,...l){return c.map((a,s)=>a+(l[s]||"")).join("")}var b=document.createElement("template");b.innerHTML=w`
    <style>
        :host{           
...
`;var u=class extends HTMLElement{constructor(){super();h(this,"config",{min:0,max:100,shape:"rect",rivets:!0,led:!0,scales:[],measurement:"",unit:"",multiplier:0,digits:{size:100,distance:14},zones:[],platehue:0,size:"100%"});h(this,"lastValue");h(this,"width");let a=this.attachShadow({mode:"open",delegatesFocus:!0});a.append(b.content.cloneNode(!0)),this.dispatchEvent(new Event(`${f}:construction`,{bubbles:!0,composed:!0})),this.wrapper=document.createElement("div"),a.appendChild(this.wrapper)}static get observedAttributes(){return["min","max","shape","multiplier","measurement","unit","rivets","digits","led","zones","platehue","size"]}connectedCallback(){this.draw()}attributeChangedCallback(a,s,t){if(s!==t&&this.config.hasOwnProperty(a)){switch(a){case"min":{t=parseFloat(t),isNaN(t)&&(t=0);break}case"max":{t=parseFloat(t),isNaN(t)&&(t=100);break}case"platehue":{if(console.log(isNaN(t),getComputedStyle(document.body).getPropertyValue(t)),isNaN(t)){let n=getComputedStyle(document.body).getPropertyValue(t);n===""?t=this.config.platehue:t=n}break}case"multiplier":{t=parseFloat(t),(isNaN(t)||t==0)&&(t=!1);break}case"led":case"rivets":t=t=="true";case"digits":{try{t=JSON.parse(t)}catch(n){console.log(n),t=this.config.digits}break}case"zones":{try{t=JSON.parse(t)}catch(n){console.warn(n,t),t=this.config.zones}break}default:break}this.config[a]=t}this.draw(),this.lastValue&&this.update(this.lastValue)}draw(){let a=(this.config.max-this.config.min)/10,s=this.config.min;this.config.scales=[];let t=this.config.multiplier;for(let e=0;e<11;++e)this.config.scales.push(s),s=parseFloat((s+a).toFixed(2));t&&t!=0&&(this.config.scales=this.config.scales.map(e=>parseFloat((e/t).toFixed(2)))),this.wrapper.replaceChildren(),this.wrapper.style.width=this.config.size,this.wrapper.style.height=this.config.size,this.wrapper.style.position="relative",this.width=this.wrapper.getBoundingClientRect().width,this.gauge=document.createElement("div"),this.gauge.setAttribute("style","--gauge-value:0;--ga-tick-count:10;--ga-subtick-count:100;"),this.gauge.style.width="100%",this.gauge.style.setProperty("height","100%"),this.gauge.style.setProperty("--container-size",this.width/50),this.gauge.style.setProperty("--gn-distance",this.config.digits.distance),this.gauge.style.setProperty("--digit-size",this.config.digits.size),this.gauge.style.setProperty("--hng-plate-hue",this.config.platehue),this.gauge.style.setProperty("--plate-hue",this.config.platehue);let n=document.createElement("div");n.className="g-body",this.config.shape=="round"&&n.classList.add("g-round"),this.gauge.appendChild(n);let g=document.createElement("div");if(g.className="g-ring",n.appendChild(g),this.config.rivets&&this.config.shape=="rect"){let e=document.createElement("div");e.className="g-rivets",g.appendChild(e);for(let i=0;i<4;++i){let o=document.createElement("div");o.className="g-rivet",e.appendChild(o)}}let r=document.createElement("div");if(r.className="g-plate",g.appendChild(r),this.config.zones.length>0){let e,i;this.config.zones.forEach(o=>{e=document.createElement("div"),i="g-zone ",i+="g-zone-"+o.cover+" ",i+=o.type,e.className=i,e.style.rotate=o.rotate+"deg",r.appendChild(e)})}this.config.led&&(this.led=document.createElement("div"),this.led.className="g-led",r.appendChild(this.led));let d=document.createElement("div");d.className="g-ticks",r.appendChild(d);for(let e=1;e<12;e++){let i=document.createElement("div");i.className="g-tick",i.setAttribute("style","--ga-tick:"+e),d.appendChild(i)}for(let e=2;e<101;e++){let i=e.toString();if(i.charAt(i.length-1)=="1")continue;let o=document.createElement("div");o.className="g-subtick",o.setAttribute("style","--ga-tick:"+e),d.appendChild(o)}let p=document.createElement("div");p.className="g-nums",r.appendChild(p);for(let e=1;e<12;e++){let i=document.createElement("div");i.className="g-num",i.setAttribute("style","--ga-tick:"+e),i.textContent=this.config.scales[e-1].toString(),p.appendChild(i)}if(this.config.measurement){let e=document.createElement("div");e.className="g-text g-label",e.textContent=this.config.measurement,r.appendChild(e)}if(this.config.unit){let e=document.createElement("div");e.className="g-text g-unit",e.textContent=this.config.unit,r.appendChild(e)}if(this.config.multiplier){let e=document.createElement("div");e.className="g-text g-multi",e.textContent="x"+this.config.multiplier,r.appendChild(e)}let m=document.createElement("div");m.className="g-needle",r.appendChild(m);let v=document.createElement("div");v.className="g-needle-ring",r.appendChild(v),this.valueField=document.createElement("div"),this.valueField.className="g-val",r.appendChild(this.valueField),this.wrapper.appendChild(this.gauge)}removeBlink(){this.led.classList.remove("active"),this.delay=null}update(a){if(this.lastValue=a,!this.valueField)return;let s=this.config.multiplier?(a/this.config.multiplier).toFixed(1):a.toFixed(1);this.valueField.textContent=s;let t=(a-this.config.min)/(this.config.max-this.config.min)*100;this.gauge.style.setProperty("--gauge-value",t),this.config.led&&(this.delay!=null&&(clearTimeout(this.delay),this.removeBlink()),this.led.classList.add("active"),this.delay=setTimeout(()=>this.removeBlink(),800))}};customElements.define(f,u);})();
//# sourceMappingURL=gauge-hotnipi.iife.min.js.map
